 ...
 *   alternating feature rows each with three proof bullets and a
 *   phone-frame vignette (schedule editor, streak-freeze callout, smart
 *   nudge notifications), a ratings summary with a 5-row histogram plus
 *   five review cards, four count-up stats, five FAQ entries, a 21x21
 *   schematic QR pattern computed from a fixed formula, and footer
 *   columns)
 * @output Desktop-first marketing landing page ABOUT a mobile app: a
 *   sticky navbar whose anchors smooth-scroll to real sections (with
 *   scroll-spy highlighting; links collapse behind a menu button at
 *   compact widths), a split hero pairing copy + store pseudo-badges with
 *   a CSS phone frame whose screen crossfades through 3 app screens every
 *   3s (dots below jump directly; reduced motion pins the first screen),
 *   a tinted band of three alternating feature rows with phone-frame
 *   vignettes, a ratings wall (4.9 average card + histogram + five review
 *   cards), a scheme-locked emerald stats band whose four numbers count
 *   up on first view, a controlled FAQ accordion, and a download band
 *   with a schematic QR card + validating email-me-a-link capture that
 *   flips to a confirmation state. Every dead-end CTA fires a corner
 *   Toast so wiring is provable; section entrances rise+fade 12px via
 *   IntersectionObserver (fire once, render visible under reduced
 *   motion).
 ...
 * is position:sticky top:0. Sections alternate full-bleed tinted bands
 * (features, stats, download, footer) with plain bands (hero, reviews,
 * FAQ); each band centers its own 1120px inner column. The Toast sits
 * fixed bottom-right.
 *
 ...
 *   last anchor above the fold line (aria-current on the active link).
 *   At compact widths the links fold behind a 40px menu button whose
 ...
 *   the cycle; prefers-reduced-motion disables the interval and the fade
 *   (static first screen, dots still swap instantly).
 ...
 * - Stats count up 0 → target with an ease-out cubic rAF loop the first
 *   time the band enters the viewport; reduced motion (and rAF-less
 *   environments) snap to the fixture values.
 ...
 * tints derived from the same two hex values; everything else uses
 * var(--color-*) tokens or neutral light-dark() pairs. The stats band
 * and footer are deliberately scheme-locked dark surfaces
 * (colorScheme:'dark' + literal emerald-dark gradients) so the brand
 * moment reads identically in both app themes; DARK_TEXT* literals only
 * ever sit on those locked surfaces. No network images, no real store
 * marks — store badges are bordered pseudo-badges, the QR is a schematic
 * grid, and app screens are CSS/SVG compositions.
 *
 * Responsive contract (useElementWidth on the scroll container — the
 * inline demo stage is ~1045px wide, so viewport media queries are not
 * used for layout):
 * - >860px: navbar shows inline anchor links + CTA.
 * - <=860px: links collapse behind the menu button dropdown.
 * - >780px: hero is split copy/phone; feature rows alternate vignette
 *   left and right.
 * - <=780px: hero stacks (phone centered below copy) and every feature
 *   row stacks in source order; the reviews and stats grids drop
 *   3→2→1 and 4→2→1 via Grid minWidth.
 * - <=560px: headline and stat sizes step down, the email capture and
 *   QR card stack, store badges go full-width, and section paddings
 *   tighten. Holds at 390px in the phone artboard with no overflow-x.
 * - Tap targets: nav links, menu button, and dots-row buttons are
 *   explicit 40px/24px controls; nothing on the page requires hover.
 */
...
// #4ADE80 ≈ 9.8:1. The tint/border constants below are alpha ramps of
// the same two hex values, not new hues.
...
const styles: Record<string, CSSProperties> = {
  // Scroll container: owns scroll-spy, width measurement, and hosts the
  // sticky navbar.
  page: {
    position: 'relative',
    height: '100%',
  ...
  // Each band centers this inner column; tinted bands paint full-bleed.
  inner: {
    width: '100%',
    maxWidth: 1120,
    marginInline: 'auto',
  ...
  phoneFrame: {
    ...
    boxShadow: 'var(--shadow-high, 0 24px 48px rgba(2, 44, 34, 0.18))',
    overflow: 'hidden',
    flexShrink: 0,
  ...
  notifCard: {
    ...
    border: '1px solid var(--color-border)',
    backgroundColor: 'var(--color-background-card)',
    boxShadow: 'var(--shadow-low, 0 2px 8px rgba(15, 23, 42, 0.08))',
  },
  notifBody: {
    fontSize: 11,
    lineHeight: 1.45,
  ...
  // ---- stats band (scheme-locked emerald dark) ----
  statsBand: {
    colorScheme: 'dark',
    color: DARK_TEXT,
    backgroundImage: [
      'radial-gradient(70% 90% at 85% 0%, rgba(74, 222, 128, 0.22), transparent 55%)',
  ...
  statCell: {
    ...
    borderLeft: \`2px solid \${DARK_CHIP_BORDER}\`,
  },
  darkChip: {
    display: 'inline-flex',
    alignItems: 'center',
...
const HERO = {
  ...
  headline: 'Small steps, kept daily',
  subcopy:
 ...
 */
function useCountUp(target: number, isActive: boolean, durationMs = 1400): number {
...
/** Scroll-reveal wrapper: rise+fade 12px once; reduced motion = visible. */
function Reveal({children, delayMs = 0}: {children: ReactNode; delayMs?: number}) {
  const [ref, isInView] = useInView();
  const isReduced = usePrefersReducedMotion();
  const isShown = isInView || isReduced;
  return (
    <div
      ref={ref}
      style={{
...
/** Accent check bullet used by the feature rows. */
...
export default function MobileAppLandingTemplate() {
  ...
  const onPageScroll = (event: UIEvent<HTMLDivElement>) => {
    ...
    for (const anchor of NAV_ANCHORS) {
      ...
      if (section != null && section.offsetTop <= container.scrollTop + SPY_OFFSET) {
        active = anchor.id;
      }
    }
  ...
  const innerStyle = {
    ...styles.inner,
    ...(isPhone ? styles.innerCompact : null),
  };
  ...
  // ---- hero: copy + store badges + crossfading phone ----
      ...
      <div style={{...styles.heroRow, ...(isStacked ? styles.heroRowStacked : null)}}>
        <div style={styles.heroText}>
          <HStack gap={2} vAlign="center" wrap="wrap">
            ...
            <Badge variant="success" label="4.9★ · 38k ratings" />
          </HStack>
          <h1
            style={{
              ...styles.heroHeadline,
      ...
      <div style={innerStyle}>
        <VStack gap={isStacked ? 6 : 8}>
          <Reveal>
            <SectionIntro
              kicker="Why Stride"
              title="Built for the days you almost skip"
              description="Three ideas carry the whole app: flexible plans, forgiving streaks, and nudges with manners."
            />
          </Reveal>
            ...
            <Reveal key={row.id} delayMs={isStacked ? 0 : 80}>
              <div
                ...
                <div
                  ...
                  <PhoneFrame
                    isSmall
                    label={\`Stride app vignette: \${row.title}\`}>
  ...
  // ---- ratings wall ----
  const reviews = (
    <section ref={registerSection('reviews')} aria-label="Reviews">
      <div style={innerStyle}>
        <VStack gap={5}>
          <Reveal>
            <SectionIntro
              kicker="Reviews"
              title="4.9 stars, earned one streak at a time"
              description="Pulled from this month's store reviews — freezes, flexible plans, and the widget come up a lot."
            />
          </Reveal>
          <Reveal delayMs={80}>
            <Grid columns={{minWidth: 280, max: 3}} gap={3}>
              <Card padding={5} height="100%">
                <VStack gap={3}>
                  <HStack gap={3} vAlign="center">
                    <span style={styles.ratingBig}>{RATING_SUMMARY.average}</span>
                    <VStack gap={1}>
              ...
              {REVIEWS.map(review => (
                <Card key={review.id} padding={5} height="100%">
                  <VStack gap={3}>
                    <HStack gap={2} vAlign="center">
  ...
  // ---- stats band (scheme-locked emerald; counters arm on first view) ----
  const statsBand = (
    <section aria-label="Stride by the numbers" style={styles.statsBand}>
      <div ref={statsRef} style={innerStyle}>
        <VStack gap={5}>
          ...
          <Grid columns={{minWidth: isPhone ? 200 : 220, max: 4}} gap={4}>
            {STATS.map(stat => (
              <StatCell
                key={stat.id}
                stat={stat}
        ...
        <VStack gap={4}>
          <Reveal>
            <SectionIntro
              ...
              description="The five questions every new Strider asks, answered without fine print."
            />
          </Reveal>
          <Reveal delayMs={80}>
            <Card padding={5}>
  ...
  // ---- download band: QR card + email capture ----
  const emailCapture =
    confirmedEmail !== null ? (
      ...
      <div style={innerStyle}>
        <VStack gap={4}>
          <Reveal>
            <SectionIntro
              kicker="Download"
              title="Get Stride on your phone"
              description="Scan the code with your camera, or send yourself a link — day 1 of the streak starts tonight."
            />
          </Reveal>
          <Reveal delayMs={80}>
            <Card padding={6}>
              <div style={{...styles.qrCard, ...(isPhone ? styles.qrCardStacked : null)}}>
                ...
                <StackItem size="fill">
                  <VStack gap={3}>
                    <Heading level={3}>Or email yourself the link</Heading>
                    <Text type="body" color="secondary">
      ...
      <Layout
        ...
        content={
          <LayoutContent padding={0} role="main" label="Stride landing page">
            {/* Scroll container: owns scroll-spy + width measurement;
                the navbar inside it is sticky. */}
