...
const styles = stylex.create({
  ...
  seat: {
    ...
    strokeWidth: 0.8,
    stroke: 'color-mix(in srgb, var(--color-text-primary) 16%, transparent)',
    outlineWidth: '2px',
    outlineOffset: '2px',
  ...
  seatInteractive: {
    cursor: 'pointer',
    transform: {default: 'scale(1)', ':hover': 'scale(1.35)'},
  },
  // Four-step price ramp over one chart token so it re-derives per scheme.
  tierPremium: {
    fill: {
      default:
        'color-mix(in oklab, var(--color-data-categorical-green) 88%, var(--color-background-body))',
      ':hover':
        'color-mix(in oklab, var(--color-text-primary) 20%, color-mix(in oklab, var(--color-data-categorical-green) 88%, var(--color-background-body)))',
    },
  },
  tierPreferred: {
    fill: {
      default:
        'color-mix(in oklab, var(--color-data-categorical-green) 62%, var(--color-background-body))',
      ':hover':
        'color-mix(in oklab, var(--color-text-primary) 20%, color-mix(in oklab, var(--color-data-categorical-green) 62%, var(--color-background-body)))',
    },
  },
  tierStandard: {
    fill: {
      default:
        'color-mix(in oklab, var(--color-data-categorical-green) 42%, var(--color-background-body))',
      ':hover':
        'color-mix(in oklab, var(--color-text-primary) 20%, color-mix(in oklab, var(--color-data-categorical-green) 42%, var(--color-background-body)))',
    },
  },
  tierValue: {
    fill: {
      default:
        'color-mix(in oklab, var(--color-data-categorical-green) 24%, var(--color-background-body))',
      ':hover':
        'color-mix(in oklab, var(--color-text-primary) 20%, color-mix(in oklab, var(--color-data-categorical-green) 24%, var(--color-background-body)))',
    },
  },
  seatHeld: {
  ...
  seatSold: {
    fill: {
      default:
        'color-mix(in oklab, var(--color-text-secondary) 24%, var(--color-background-body))',
      ':hover':
        'color-mix(in oklab, var(--color-text-secondary) 24%, var(--color-background-body))',
    },
    cursor: 'default',
  ...
  seatCompanion: {
    fill: {
      default: 'color-mix(in oklab, var(--color-accent) 16%, var(--color-background-body))',
  ...
  legendPanel: {
    ...
    backgroundColor: 'var(--color-background-body)',
    boxShadow:
      '0 8px 24px light-dark(rgba(20, 24, 32, 0.14), rgba(0, 0, 0, 0.5))',
    zIndex: 2,
  ...
  swatch: {
    ...
    borderWidth: '1px',
    borderStyle: 'solid',
    borderColor: 'color-mix(in srgb, var(--color-text-primary) 16%, transparent)',
  },
  swatchPremium: {
    backgroundColor:
      'color-mix(in oklab, var(--color-data-categorical-green) 88%, var(--color-background-body))',
  ...
  swatchCompanion: {
    backgroundColor:
      'color-mix(in oklab, var(--color-accent) 16%, var(--color-background-body))',
    borderStyle: 'dashed',
  ...
  sheet: {
    ...
    backgroundColor: 'var(--color-background-body)',
    boxShadow:
      '0 -8px 28px light-dark(rgba(20, 24, 32, 0.16), rgba(0, 0, 0, 0.55))',
    overflow: 'hidden',
  ...
  return (
    <VStack gap={1}>
      ...
      <div {...stylex.props(styles.legendRow)}>
        <span {...stylex.props(styles.swatch, styles.swatchSelected)} />
        <Text type="supporting" size="sm" color="secondary">
          In your cart
        </Text>
      </div>
      <div {...stylex.props(styles.legendRow)}>
        <span {...stylex.props(styles.swatch, styles.swatchHeld)} />
        <Text type="supporting" size="sm" color="secondary">
          Held by another buyer
        </Text>
      </div>
      <div {...stylex.props(styles.legendRow)}>
        <span {...stylex.props(styles.swatch, styles.swatchSold)} />
        <Text type="supporting" size="sm" color="secondary">
          Sold
        </Text>
      </div>
      <div {...stylex.props(styles.legendRow)}>
        <span {...stylex.props(styles.swatch, styles.swatchCompanion)} />
    ...
    <VStack gap={3}>
      ...
      <VStack gap={1}>
        ...
        <HStack gap={2} vAlign="center">
          <StackItem size="fill">
            <Text type="supporting" color="secondary">
              Service fees · {seatTotal} × {formatMoney(SERVICE_FEE)}
            </Text>
          </StackItem>
          <Text type="supporting" hasTabularNumbers>
            {formatMoney(fees)}
    ...
    <div {...stylex.props(styles.mapShell)}>
      <svg
        ...
        <g
          ...
          {/* Section hulls: tap targets for zoom + the section silhouettes. */}
          {SECTION_HULLS.map(hull => (
            <rect
              key={hull.id}
              x={hull.view.x}
      ...
      <div {...stylex.props(styles.zoomControls)}>
        <button
          type="button"
          aria-label="Zoom in"
          disabled={phoneZoomIdx >= PHONE_ZOOMS.length - 1}
          onClick={() => stepPhoneZoom(1)}
          {...stylex.props(
            styles.zoomButton,
            phoneZoomIdx >= PHONE_ZOOMS.length - 1 && styles.zoomButtonDisabled,
          )}>
          <Icon icon={ZoomInIcon} size="sm" color="inherit" />
        </button>
        <button
          type="button"
          aria-label="Zoom out"
          disabled={phoneZoomIdx === 0}
          onClick={() => stepPhoneZoom(-1)}
          {...stylex.props(
            styles.zoomButton,
            phoneZoomIdx === 0 && styles.zoomButtonDisabled,
          )}>
          <Icon icon={ZoomOutIcon} size="sm" color="inherit" />
        </button>
        <button
          type="button"
